 ...
 *
 *
 * You can add and remove views using the normal methods [ViewGroup.addView],
 * [ViewGroup.removeView] etc. You may want to listen for updates by calling
 * [HorizontalPager.setOnScreenSwitchListener] in order to perform
 ...
 *
 * @author Marc Reichelt, [http://www.marcreichelt.de/](http://www.marcreichelt.de/)
 * @version 0.1.0
 */
class HorizontalPager : ViewGroup {
    ...
    private var mOnScreenSwitchListener: OnScreenSwitchListener? = null
    ...
    private var mLastSeenLayoutWidth = -1
    ...
    private fun init() {
        ...
        mDensityAdjustedSnapVelocity = (displayMetrics.density * SNAP_VELOCITY_DIP_PER_SECOND).toInt()
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        kotlin.check(widthMode == MeasureSpec.EXACTLY) { "ViewSwitcher can only be used in EXACTLY mode." }

        val heightMode = View.MeasureSpec.getMode(heightMeasureSpec)
        kotlin.check(heightMode == MeasureSpec.EXACTLY) { "ViewSwitcher can only be used in EXACTLY mode." }
        ...
        if (mFirstLayout) {
            ...
            mNextScreen = Math.max(0, Math.min(currentScreen, childCount - 1))
            val newX = mNextScreen * displayWidth
        ...
        when (action) {
            ...
            MotionEvent.ACTION_UP -> {
                if (mTouchState == TOUCH_STATE_HORIZONTAL_SCROLLING) {
                    ...
                    if (velocityX > mDensityAdjustedSnapVelocity && currentScreen > 0) {
                        // Fling hard enough to move left
                        snapToScreen(currentScreen - 1)
                    } else if (velocityX < -mDensityAdjustedSnapVelocity && currentScreen < childCount - 1) {
    ...
    override fun computeScroll() {
        if (mScroller!!.computeScrollOffset()) {
            ...
            currentScreen = Math.max(0, Math.min(mNextScreen, childCount - 1))
     ...
     */
    fun setCurrentScreen(currentScreen: Int, animate: Boolean) {
        this.currentScreen = Math.max(0, Math.min(currentScreen, childCount - 1))
        if (animate) {
     ...
     */
    fun setOnScreenSwitchListener(onScreenSwitchListener: OnScreenSwitchListener) {
    ...
    private fun snapToDestination() {
        ...
        val deltaX = scrollX - screenWidth * currentScreen
        ...
        } else if (deltaX > 0 && currentScreen + 1 != childCount
                && screenWidth / FRACTION_OF_SCREEN_WIDTH_FOR_SWIPE < deltaX) {
            whichScreen++
        }
     ...
     */
    private fun snapToScreen(whichScreen: Int, duration: Int = -1) {
        /*
        ...
        val newX = mNextScreen * width
        val delta = newX - scrollX

        if (duration < 0) {
            // E.g. if they've scrolled 80% of the way, only animation for 20% of the duration
            mScroller!!.startScroll(scrollX, 0, delta, 0, (Math.abs(delta) / width.toFloat() * ANIMATION_SCREEN_SET_DURATION_MILLIS).toInt())
        } else {
    ...
    interface OnScreenSwitchListener {
         ...
         */
        fun onScreenSwitched(screen: Int)
    }
     ...
     */
        private val ANIMATION_SCREEN_SET_DURATION_MILLIS = 500
        // What fraction (1/x) of the screen the user must swipe to indicate a page change
        private val FRACTION_OF_SCREEN_WIDTH_FOR_SWIPE = 4
     ...
     */
        private val SNAP_VELOCITY_DIP_PER_SECOND = 600
